...
| **Authoriser** | `P4` *Authorise WIP information for controlled sharing* — `TTL` **Au**. `D4` *Authorise publication / exchange* — **`TBD`** against `AP`, `LDP`, `BM`, `BC` and `TTL`; **`—`** against `Aut`, `Chk`, `CDE` and `Rcp`. **Publication / exchange authority is unresolved and no allocation is made.** `A2` *Authorise governance change* — **`TBD`**, and *"**No single universal approver exists**"* |
